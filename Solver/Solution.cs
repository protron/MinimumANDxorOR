...
public class Solution
{
    public static void Main()
    {
        ...
        while (t-- > 0)
        {
            ...
            var values = line.Split(new[] { ' ' }).Select(x => int.Parse(x));
            var kCombs = values.Subsets(2);
            var result = int.MaxValue;
            foreach (var x in kCombs)
            {
                var ai = x.First();
                var aj = x.Last();
                var current = (ai & aj) ^ (ai | aj);
                if (current < result)
                {
                    result = current;
                }
...
public static partial class MoreEnumerable
{
    ...
    public static IEnumerable<IList<T>> Subsets<T>(this IEnumerable<T> sequence)
    {
        ...
        return SubsetsInternal(sequence);
    }

    private static IEnumerable<IList<T>> SubsetsInternal<T>(IEnumerable<T> sequence)
    {
